...
| `<script>` restante | UTIL → CSV PARSING → TOPOJSON+PROJEÇÃO → ESTADO → TOOLTIP → INTAKE/IO → CONTROLES → KPIs → MAPA → MATRIZ → DISPERSÃO → RANKING → CONTINENTE → HISTOGRAMA → r POR CONTINENTE → TABELA → RENDER |
